...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    const parsedBg = bgColor ? new THREE.Color(bgColor) : null;
    const bgVec4 = parsedBg
      ? new THREE.Vector4(parsedBg.r, parsedBg.g, parsedBg.b, 1.0)
    ...
    class CommonClass {
      ...
      init(container: HTMLElement) {
        ...
        this.resize();
        this.renderer = new THREE.WebGLRenderer({ antialias: false, alpha: !parsedBg });
        this.renderer.autoClear = false;
        if (parsedBg) {
          this.renderer.setClearColor(parsedBg, 1);
        } else {
...
`;
    ...
    class Advection extends ShaderPass {
      ...
      update(...args: unknown[]) {
        const { dt: dt_, isBounce: isBounce_, BFECC: BFECC_ } = (args[0] || {}) as { dt?: number; isBounce?: boolean; BFECC?: boolean };
        if (!this.uniforms) return;
        if (typeof dt_ === 'number') this.uniforms.dt.value = dt_;
        if (typeof isBounce_ === 'boolean') this.line.visible = isBounce_;
        if (typeof BFECC_ === 'boolean') this.uniforms.isBFECC.value = BFECC_;
        super.update();
      }
    }
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: unknown[]) {
        ...
        const cursorSize_ = (props.cursor_size as number) || 0;
        const cursorSizeX = cursorSize_ * cellScale.x;
        const cursorSizeY = cursorSize_ * cellScale.y;
        ...
        uniforms.scale.value.set(cursorSize_, cursorSize_);
        super.update();
      }
    }
    ...
    class Viscous extends ShaderPass {
      ...
      update(...args: unknown[]) {
        const { viscous: v_, iterations, dt: dt_ } = (args[0] || {}) as { viscous?: number; iterations?: number; dt?: number };
        if (!this.uniforms) return;
        let fbo_in: THREE.WebGLRenderTarget, fbo_out: THREE.WebGLRenderTarget;
        if (typeof v_ === 'number') this.uniforms.v.value = v_;
  ...
  }, [BFECC, cursorSize, dt, isBounce, isViscous, iterationsPoisson, iterationsViscous, mouseForce, resolution, viscous, colors, bgColor, autoDemo, autoSpeed, autoIntensity, takeoverDuration, autoResumeDelay, autoRampDuration]);
