...
function ProfileContent() {
  ...
  return (
    <div className="mx-auto w-full max-w-[1440px] space-y-6">
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[2fr_0.95fr]">
        <div className="space-y-5">
          <section className="rounded-2xl border border-white/40 shadow-sm" style={{background:'rgba(255,255,255,0.72)',backdropFilter:'blur(14px)',WebkitBackdropFilter:'blur(14px)'}}>
            <div className="h-24 rounded-t-2xl bg-gradient-to-r from-emerald-200/60 via-emerald-100/40 to-emerald-200/60" />
            <div className="-mt-10 flex flex-wrap items-start justify-between gap-4 px-6 pb-6">
              ...
              <button type="button" onClick={() => setEditOpen(true)} className="mt-7 inline-flex h-10 items-center gap-2 rounded-xl border border-white/40 bg-white/50 px-4 text-sm font-medium text-slate-700 hover:bg-white/70">
          ...
          <section className="rounded-2xl border border-white/40 p-5 shadow-sm" style={{background:'rgba(255,255,255,0.72)',backdropFilter:'blur(14px)',WebkitBackdropFilter:'blur(14px)'}}>
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              {connections.length > 0 ? connections.map((person) => (
                <div key={person.userId} className="flex items-center justify-between rounded-xl border border-white/30 bg-white/50 p-3">
                ...
                <div className="col-span-full rounded-xl border border-white/30 bg-white/40 px-4 py-5 text-sm text-slate-500">
        ...
        <aside className="space-y-4">
          ...
          <section className="rounded-2xl border border-white/40 p-5 shadow-sm" style={{background:'rgba(210,242,220,0.72)',backdropFilter:'blur(14px)',WebkitBackdropFilter:'blur(14px)'}}>
            <div className="flex items-center justify-between">
              <h2 className="text-[18px] font-semibold text-slate-900">NEXT RIDE</h2>
              <span className="rounded bg-white/60 px-2 py-0.5 text-[10px] font-semibold text-slate-600">TODAY</span>
            </div>
            {nextRide ? (
              <>
      ...
      {loading && (
        <div className="rounded-xl border border-white/40 px-4 py-3 text-sm text-slate-500" style={{background:'rgba(255,255,255,0.72)',backdropFilter:'blur(14px)',WebkitBackdropFilter:'blur(14px)'}}>Loading profile details…</div>
      )}
...
function EditProfileModal({
  ...
  return (
    ...
    >
      <div className="w-full max-w-md rounded-2xl border border-white/40 shadow-2xl" style={{background:'rgba(255,255,255,0.82)',backdropFilter:'blur(20px)',WebkitBackdropFilter:'blur(20px)'}}>
        {/* Header */}
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-6 pb-6 pt-4">
          ...
          <div>
            ...
            <input
              ...
              className="h-11 w-full rounded-xl border border-white/40 bg-white/50 px-3 text-sm text-slate-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-200"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              value={user.email}
              readOnly
              className="h-11 w-full rounded-xl border border-white/30 bg-white/30 px-3 text-sm text-slate-500 cursor-not-allowed"
            />
          </div>
          ...
          <div className="flex gap-3 pt-1">
            <button
              ...
              onClick={onClose}
              disabled={saving}
              className="flex-1 h-11 rounded-xl border border-white/40 bg-white/50 text-sm font-medium text-slate-700 hover:bg-white/70 disabled:opacity-50"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={saving}
              className="flex-1 h-11 inline-flex items-center justify-center gap-2 rounded-xl bg-brand-500 text-sm font-semibold text-slate-900 hover:bg-brand-400 disabled:opacity-60"
            >
...
function MetricCard({
  ...
  return (
    <div className="rounded-2xl border border-white/40 p-4 text-center shadow-sm" style={{background:'rgba(255,255,255,0.72)',backdropFilter:'blur(14px)',WebkitBackdropFilter:'blur(14px)'}}>
      ...
      <div className="mb-4 flex items-center justify-between">
        ...
        {!editing && (
          <button type="button" onClick={() => setEditing(true)}
            className="inline-flex h-8 items-center gap-1.5 rounded-xl border border-white/40 bg-white/50 px-3 text-xs font-medium text-slate-600 hover:bg-white/70">
        ...
        <div>
          ...
          {editing
            ? <input value={model} onChange={(e) => setModel(e.target.value)} placeholder="e.g. Tesla Model 3"
                className="h-12 w-full rounded-xl border border-white/40 bg-white/50 px-3 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-200" />
            : <div className="flex h-12 items-center rounded-xl border border-white/30 bg-white/40 px-3 text-sm text-slate-700">
        ...
        <div className="mt-4 space-y-2">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={handleCancel} disabled={saving}
              className="flex-1 h-9 rounded-xl border border-white/40 bg-white/50 text-sm font-medium text-slate-700 hover:bg-white/70 disabled:opacity-50">Cancel</button>
            <button type="button" onClick={handleSave} disabled={saving}
              className="flex-1 h-9 inline-flex items-center justify-center gap-1.5 rounded-xl bg-brand-500 text-sm font-semibold text-slate-900 hover:bg-brand-400 disabled:opacity-60">
...
function VerificationSection({
  ...
  return (
    <section className="rounded-2xl border border-white/40 p-5 shadow-sm" style={{background:'rgba(255,255,255,0.72)',backdropFilter:'blur(14px)',WebkitBackdropFilter:'blur(14px)'}}>
            ...
            {editing ? (
              <div className="mt-1 space-y-1">
                <input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+91 9876543210"
                  className="h-9 w-full rounded-xl border border-white/40 bg-white/50 px-2 text-xs text-slate-900 focus:outline-none focus:ring-2 focus:ring-brand-200" />
      ...
      <div className="mt-4 rounded-xl border border-white/30 bg-white/40 p-3">
    ...
    <section className="rounded-2xl border border-white/40 p-5 shadow-sm" style={{background:'rgba(255,255,255,0.72)',backdropFilter:'blur(14px)',WebkitBackdropFilter:'blur(14px)'}}>
      ...
      {editing ? (
        <textarea value={bio} onChange={(e) => setBio(e.target.value)} rows={4} maxLength={500}
          ...
          className="w-full rounded-xl border border-white/40 bg-white/50 px-3 py-2 text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-brand-200 resize-none" />
      ) : (
